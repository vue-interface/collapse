...
/* harmony default export */ var index = ({
  inserted: function inserted(el, binding, vnode) {
    ...
    var target = isFunction(binding.value) ? binding.value(el, binding, vnode) : el.getAttribute('data-target') || el.getAttribute('href');
    var targets = isString(target) ? document.querySelectorAll(target) : !isArray(target) ? [target] : target; // Set the class and attribute so it matches the Boostrap 4 spec.
    // el.classList.add('collapsed');
